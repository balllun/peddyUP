...
<c:forEach items="${reportList}" var="report">
	<tr>
		...
		<td class="text-center">${report.pedigreeId.pedigreeType.gene}</td>
		<td class="text-center">${report.pedigreeId.pedigreeType.type}<c:if 
		test="${report.pedigreeId.pedigreeType.type=='100%'}"><% count1++;%>	
		</c:if>
		<c:if 
		test="${(report.pedigreeId.pedigreeType.type=='F2') || (report.pedigreeId.pedigreeType.type=='F3')}"><% count2++;%>	
		</c:if>
	</td>
		<td class="text-center">${report.pedigreeId.pedigreeType.price}<%-- <c:if 
		test="${report.pedigreeId.pedigreeType.type=='100%'}"><%price;%>	
		</c:if></td> --%>
